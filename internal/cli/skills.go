...
var skillTargetSpecs = []skillTargetSpec{
	{Key: "agents", Label: "~/.agents/skills/ggg", path: gggskills.AgentSkillsInstallPath},
	{Key: "claude", Label: "~/.claude/skills/ggg", path: gggskills.ClaudeSkillsInstallPath},
}
...
type skillInstallItem struct {
	...
	Status gggskills.InstallStatus `json:"status,omitempty"`
...
type skillVerifyItem struct {
	Target string                 `json:"target"`
	Path   string                 `json:"path"`
	Status gggskills.VerifyStatus `json:"status,omitempty"`
	Error  string                 `json:"error,omitempty"`
}
...
// written by ggg, which makes `ggg skills install` refuse without --force.
func (r skillsVerifyResult) anyModified() bool {
	for _, item := range r.Verifications {
		if item.Error == "" && item.Status == gggskills.VerifyModified {
			return true
		}
	}
	return false
...
func (e staleSkillsError) Error() string {
	...
	if e.result.anyModified() {
		update = "`ggg skills install --force`"
	}
	return fmt.Sprintf("one or more installed skills are not in sync with this ggg version (run %s to update); set suppress_skills_notice: true in ~/.config/ggg/repositories.yaml to silence the reminder", update)
}
...
var skillsInstallCmd = &cobra.Command{
	Use:   "install",
	Short: "Install the bundled GGG skill for AI agents",
	Long: `Install the AI agent skill bundled with this ggg binary.

Destinations:
  agents    ~/.agents/skills/ggg    Codex and other Agent Skills hosts
  claude    ~/.claude/skills/ggg    Claude Code
...
func installSkillTargets(targets []skillTarget, force bool) skillsInstallResult {
	...
	for _, target := range targets {
		status, err := gggskills.Install(target.Path, force)
...
var skillsVerifyCmd = &cobra.Command{
	Use:   "verify",
	Short: "Check whether the installed GGG skills match this binary",
	Long: `Compare each installed copy of the bundled AI agent skill with the skill
this ggg binary carries, using the recorded SHA-256 digests.
...
func printSkillsVerifyResult(result skillsVerifyResult) {
	...
	for _, item := range result.Verifications {
		...
		case item.Error != "":
			fmt.Printf("  %s %s %s\n", ui.Error.Render("✗"), ui.Repo.Render(displayPath(item.Path)), ui.Error.Render(item.Error))
		case item.Status == gggskills.VerifyUpToDate:
			fmt.Printf("  %s %s %s\n", ui.Success.Render("✓"), ui.Repo.Render(displayPath(item.Path)), ui.Muted.Render(string(item.Status)))
		case item.Status == gggskills.VerifyNotInstalled:
			fmt.Printf("  %s %s %s\n", ui.Muted.Render("·"), ui.Repo.Render(displayPath(item.Path)), ui.Muted.Render(string(item.Status)))
		default:
			fmt.Printf("  %s %s %s\n", ui.Error.Render("✗"), ui.Repo.Render(displayPath(item.Path)), ui.Error.Render(string(item.Status)))
		}
	}
	fmt.Println()
}
...
func init() {
	skillsInstallCmd.Flags().Bool("force", false, "Replace an existing GGG skill that differs from the bundled version")

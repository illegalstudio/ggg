...
func TestVerifySkillTargetsReportsEachDestination(t *testing.T) {
	...
	if _, err := gggskills.Install(all[0].Path, false); err != nil {
		t.Fatal(err)
	}
	...
	if got := result.Verifications[0].Status; got != gggskills.VerifyUpToDate {
		t.Fatalf("agents status = %q, want %q", got, gggskills.VerifyUpToDate)
	}
	if got := result.Verifications[1].Status; got != gggskills.VerifyNotInstalled {
		t.Fatalf("claude status = %q, want %q", got, gggskills.VerifyNotInstalled)
	}
	if result.anyStale() {
...
func TestVerifySkillTargetsFlagsStaleDestinations(t *testing.T) {
	...
	result := verifySkillTargets(all)
	if got := result.Verifications[0].Status; got != gggskills.VerifyModified {
		t.Fatalf("agents status = %q, want %q", got, gggskills.VerifyModified)
	}
	if !result.anyStale() {
...
func TestSkillNoticeApplies(t *testing.T) {
	...
	root.AddCommand(skills, list, cd, shellInit, completion, help, complete)
...
func TestStaleSkillsErrorSuggestsForceOnlyForModified(t *testing.T) {
	outdated := skillsVerifyResult{Verifications: []skillVerifyItem{
		{Target: "agents", Status: gggskills.VerifyOutdated},
	}}
	...
	modified := skillsVerifyResult{Verifications: []skillVerifyItem{
		{Target: "agents", Status: gggskills.VerifyModified},
	}}

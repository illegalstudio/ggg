...
func TestInstallPaths(t *testing.T) {
	home := filepath.Join(string(filepath.Separator), "tmp", "ggg-home")
	tests := []struct {
		name string
		got  string
		want string
	}{
		{
			...
			want: filepath.Join(home, ".agents", "skills", "ggg"),
		},
		{
			name: "Claude Code",
			got:  ClaudeSkillsInstallPath(home),
			want: filepath.Join(home, ".claude", "skills", "ggg"),
		},
	}
...
func TestInstallLeavesNoStagingDirectories(t *testing.T) {
	parent := t.TempDir()
	destination := filepath.Join(parent, "ggg")
	if _, err := Install(destination, false); err != nil {
		t.Fatal(err)
	}
	...
	if len(entries) != 1 || entries[0].Name() != "ggg" {
